...
import {
  View, Text, StyleSheet, TouchableOpacity, Switch, ScrollView, Alert, Image, TextInput,
} from 'react-native';
...
export default function SettingsScreen() {
  const { settings, updateSettings, clearCache, exportData, isDarkMode, toggleDarkMode } =
    useBaguioQuest();
  ...
  // Load persisted values (plate + codingAlerts)
  useEffect(() => {
    (async () => {
      const savedPlate = await AsyncStorage.getItem('plateLastDigit');
      if (savedPlate) setPlate(savedPlate);
      const codingAlertsRaw = await AsyncStorage.getItem('codingAlerts');
      if (codingAlertsRaw != null) setCodingEnabled(codingAlertsRaw === 'true');
    })();
  }, []);

  const handleCodingToggle = async (v: boolean) => {
    setCodingEnabled(v);
    updateSettings({ numberCodingEnabled: v });
    await AsyncStorage.setItem('codingAlerts', String(v)); // <- used by map alerts
    if (v) await ensureNotificationSetup(); // ask for permission when enabling
  };
  ...
  const handleNotificationsToggle = async (v: boolean) => {
    ...
    // Optional: if you want global toggle to also control coding alerts, uncomment:
    // await AsyncStorage.setItem('codingAlerts', String(v));
    if (v) await ensureNotificationSetup();
  };

  const showCodingInfo = () => {
    Alert.alert(
      'Number Coding Information',
      'Default windows are based on current LGU regulations (Mon–Fri, 7:00–19:00; 1–2/Mon, 3–4/Tue, 5–6/Wed, 7–8/Thu, 9–0/Fri). Always follow official traffic signs and officers.',
      [{ text: 'OK' }]
    );
  };

  const handleSavePlate = async () => {
    const last = String(plate).trim().slice(-1);
    const digit = Number(last);
    if (Number.isNaN(digit)) {
      Alert.alert('Invalid plate', 'Enter the LAST digit of your plate (0–9).');
      return;
    }
    await AsyncStorage.setItem('plateLastDigit', String(digit));
    updateSettings({ plateLastDigit: digit } as any); // your hook may ignore unknown fields; that’s fine
    Alert.alert('Saved', `Plate last digit set to ${digit}.`);
  };
    ...
    <SafeAreaView style={styles.container}>
      ...
      <ScrollView style={styles.content} showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.section}>
          ...
          {/* Plate last digit input */}
          {codingEnabled && (
            <View style={{ paddingHorizontal: 16, paddingTop: 12, gap: 10 }}>
              <Text style={{ fontSize: 12, color: '#6b7280' }}>
                Enter the <Text style={{ fontWeight: '700', color: '#1f2937' }}>last digit</Text> of your plate
                so the app can tell if you’re restricted today.
              </Text>
              <View style={styles.plateRow}>
                ...
                <TouchableOpacity style={styles.saveBtn} onPress={handleSavePlate}>
                  <Text style={styles.saveBtnText}>Save Plate</Text>
                </TouchableOpacity>
              </View>
            </View>
          )}
            ...
            <View style={styles.codingDetails}>
              ...
              <TouchableOpacity
                ...
                onPress={() =>
                  Alert.alert(
                    'Customize Windows',
                    'In a future update, you can override times/days here. For now, edit data/coding_rules.json.'
                  )
                }
              >
        ...
        <View style={styles.section}>
          ...
          <View style={styles.disclaimer}>
            ...
            <Text style={styles.disclaimerText}>
              Informational guidance only. Always follow official signs and officers. Conditions may
